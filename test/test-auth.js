...
describe('test utilAuth.createUser', () => {
  let db;
  before(async () => {
    await User.deleteOne({ name: 'TEST_USER_DELETE_ME' });
    db = require('../db');
  });

  it('create new user', async () => {
    try {
      const result = await util.createUser('TEST_USER_DELETE_ME', '12345');
  ...
  it('try to create user without password (empty)', async () => {
    try {
      await User.deleteOne({ name: 'TEST_USER_DELETE_ME' });
      const result = await util.createUser('TEST_USER_DELETE_ME', '');
      assert.fail(); // should not reach this...
    } catch (err) {
      expect(err).to.be.an('error');
  ...
  it('try to create user without password (null)', async () => {
    try {
      await User.deleteOne({ name: 'TEST_USER_DELETE_ME' });
      const result = await util.createUser('TEST_USER_DELETE_ME', null);
      assert.fail(); // should not reach this...
    } catch (err) {
      expect(err).to.be.an('error');
      expect(err.message).to.equal('No password provided');
  ...
  describe('test utilAuth.login', () => {
    ...
    it('login with valid principal and credenital', async () => {
      try {
        await util.createUser('TEST_USER_DELETE_ME', '12345');
        await new Promise(resolve => setTimeout(resolve, 100)); // sleep a little while...

        const result = await util.login('TEST_USER_DELETE_ME', '12345');
    ...
    it('login with valid principal and invalid credenital', async () => {
      try {
        await util.createUser('TEST_USER_DELETE_ME', 'XXXXX');
        await new Promise(resolve => setTimeout(resolve, 100)); // sleep a little while...
