...
function formatDistance(distOrig) {
  const distKm = parseInt(distOrig);
  let distM = (distOrig - distKm) * 1000;
...
function calculateDistance(lat1, lon1, lat2, lon2) {
  const R = 6371; // km -> m
  const dLat = toRad(lat2 - lat1); // ;(lat2-lat1).toRad();
  const dLon = toRad(lon2 - lon1); // (lon2-lon1).toRad();
 ...
 */
exports.getNumberOfTimeEntries = callback => getNumberOfTimeEntries(callback);
