                ...
                util.getLastTimeEntry(function(err, lastTimeentry) {
                    ...
                    if(res) {
                        var reply = {
                            ...
                            deletedDoublets: deletedDoublets.count_removed_doubletts
                        };
                        console.log(reply);
                        res.send(reply);
                    }
...
exports.scheduleTasks = function() {
    ...
    //scheduler.scheduleJob({hour: 4, minute: 0}, function() {
//    scheduler.scheduleJob('5 4 * * *', function() {  // every day at 04:05
    scheduler.scheduleJob({}, function() {  // every day at 04:05
        console.log('scheduled task "dumpTimeEntry" started')
        admin.dumpTimeEntry();
    });
    console.log("job scheduler: backupTimeEntry (every day at 04:10)");
    //scheduler.scheduleJob({hour: 4, minute: 0}, function() {
    scheduler.scheduleJob('10 4 * * *', function() {  // every day at 04:10
